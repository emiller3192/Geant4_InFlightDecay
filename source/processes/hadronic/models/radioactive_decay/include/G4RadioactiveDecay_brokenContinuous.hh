...
class G4RadioactiveDecay : public G4VRestContinuousDiscreteProcess 
{
  ...
  public: // with description
    ...
    // return an energy loss based on time
    // simple relation used for testing: 1 fs = 1 keV of loss
    // return a simple energy loss calculation
    // assuming a 1 keV loss per 1 femtosecond
    // the argument decay time is assumed to be in ns
    // output is positive (should be subtracted from KE later)
    // If the input lifetime is negative, it is intendeda as a long-lived particle
    // so don't do any loss
    G4double SimpleEnergyLoss(G4double time) 
    {
      G4double total_loss = 0;
      if (time > 0) {
          G4double time_fs = time * 1e6; //convert from ns to fs
          G4double loss_rate = 1 * keV; //per femtosecond
          // G4double loss_rate = 1e30 * keV; //temporary huge loss for debugging
          total_loss = time_fs * loss_rate;
          // G4double loss_rate = (1 * keV)/(1 *1e-15 * second);
          // G4double total_loss = loss_rate * time * ns; 
        } else {
            total_loss = 0;
        }
      return total_loss;
  ...
  protected:
    ...
    G4VParticleChange* DecayItAlong(const G4Track& theTrack,
                               const G4Step&  theStep);
  ...
  private:
    ...
    G4VParticleChange* PostStepDoIt(const G4Track& theTrack,
                                    const G4Step& theStep)
      {return DecayIt(theTrack, theStep);}
// Add an along-step do it to call the decay
// Used for inflight decay
// added by E. Miller and C. Bray
    inline
    G4VParticleChange* AlongStepDoIt(const G4Track& theTrack,
                                    const G4Step& theStep)
      {return DecayItAlong(theTrack, theStep);}

    // inline implementation of AlongStep GPIL
    inline
    ...
    {
      G4double steplength =
        G4VRestContinuousDiscreteProcess::AlongStepGetPhysicalInteractionLength(track, 
          previousStepSize, currentMinimumStep, currentSafety, selection);
      return steplength;
    }
