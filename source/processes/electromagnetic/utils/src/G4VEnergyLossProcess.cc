                             ...
                             G4ForceCondition* condition)
{
  ...
  *condition = NotForced;
  G4double x = DBL_MAX;
  G4double tau = track.GetDynamicParticle()->GetDefinition()->GetPDGLifeTime();
  ...
  if(isIon) {
    // if (tau != -1) G4cout << "testing decay time for tau = " << tau << " ns   is " << ComputeDecayTime(tau) << G4endl;
  ...
  if(preStepScaledEnergy < mfpKinEnergy) {
    // debug stuff written out by E. Miller
    // G4cout << "preStepScaledEnergy: " << preStepScaledEnergy << "    " <<
    //  "integral: " << integral << G4endl;
    // get the lifetime of the particle
    // G4double tau = theParticle.GetPDGLifetime();
  ...
  if(preStepLambda > 0.0) { 
    ...
    if (theNumberOfInteractionLengthLeft < 0.0) {
      ...
      // G4cout << "Interaction length left: " << theNumberOfInteractionLengthLeft;
