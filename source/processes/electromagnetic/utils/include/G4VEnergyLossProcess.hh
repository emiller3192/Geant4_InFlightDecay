...
private:
  ...
  // adding a function to calculate the lambda based on lifetime
  inline G4double ComputeDecayTime(G4double tau);
...
inline void G4VEnergyLossProcess::ComputeLambdaForScaledEnergy(G4double e)
{
  ...
  // G4cout << "e: " << e << "    " << "mfpKinEnergy: " << mfpKinEnergy << G4endl;
  if (e <= mfpKinEnergy) {
    preStepLambda = GetLambdaForScaledEnergy(e);
    // Debug info written out by E. Miller 
    // G4cout << "In the 'Get' branch. Lambda: " << preStepLambda << G4endl;

  } else {
    // G4cout << "In the 'else' branch." << G4endl;
    ...
    // G4cout << "\t" << "preStepLambda: " << preStepLambda << G4endl;
  }
}

// ======== Calculate the decay time of a particle psuedo-randomly ========
// ======== written by E. Miller 09/30/2019 
// ======== arguments: mean lifetime of the particle (units of the decay time
// match the units of tau)
inline G4double G4VEnergyLossProcess::ComputeDecayTime(G4double tau) {
// a tau of -1 indicates a stable particle, so use DBL_MAX for the decay time
  if (tau != -1) {
