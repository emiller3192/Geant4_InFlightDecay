...
G4NuclideTable::G4NuclideTable()
  ...
  threshold_of_half_life(1e-20*ns),
   ...
   flevelTolerance(1.0*eV)
{
  ...
  // checking the minimum_threshold_of_half_life
  // G4cout << "Minimum halflife value is: " << minimum_threshold_of_half_life << G4endl;
...
void G4NuclideTable::GenerateNuclide()
{
   ...
   if ( threshold_of_half_life < minimum_threshold_of_half_life ) {
      ...
      // access the IonTable to update the PDGLifeTime for the excited ions
      // G4IonTable* ionTable = (G4IonTable*)(G4ParticleTable::GetParticleTable()->GetIonTable());
      // G4cout << "Ion table ready?    " << (G4IonTable*)(G4ParticleTable::GetParticleTable()->GetReadiness()) << G4endl;; 
      ...
      while ( ifs.good() ) {// Loop checking, 09.08.2015, K.Kurashige
         ...
         if (ionZ == 4 && ionA == 10) {
            G4cout << ionZ << "\t" << ionA  << "\t" << ionE  << "\t" << ionLife << G4endl;
            bool thresh_check = threshold_of_half_life <= ionLife*std::log(2.0);
            bool min_check = ionLife*std::log(2.0) < minimum_threshold_of_half_life;
            bool energy_check = ionE != 0;
            G4cout << "Checks are (thresh, min, energy) when threshold is:" << threshold_of_half_life << G4endl;
            G4cout << "\t" << thresh_check << "  ionLife is: " << ionLife*ns << G4endl;
            G4cout << "\t" << min_check << G4endl;
            G4cout << "\t" << energy_check << G4endl;
         }
         ...
         if ( ( ionE == 0 && minimum_threshold_of_half_life == DBL_MAX ) // ground state is alwyas build in very first attempt
           || ( threshold_of_half_life <= ionLife*std::log(2.0) && ionLife*std::log(2.0) < minimum_threshold_of_half_life && ionE != 0 ) ) {
            ...
            // access the ion in the ionTable
            // G4ParticleDefinition* ion = ionTable->GetIon(ionZ, ionA, ionE);
            // set the lifetime
            // ion->SetPDGLifeTime(ionLife);
      ...
      G4cout << "Updated minimum half life threshold: " << minimum_threshold_of_half_life << G4endl;
