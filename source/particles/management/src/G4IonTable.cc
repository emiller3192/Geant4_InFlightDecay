...
//G4cout<<"G4IonTable::CreateIon "<<"Z:"<<Z<<" A:"<<A<<" E:"<<E<<" Eex:"<<Eex<<" lvl:"<<lvl<<" name:"<<name<<" code:"<<encoding<<G4endl;
  ...
  G4double stateWidth = CalcWidth(lifetime);

  ion = new G4Ions(   name,            mass,       stateWidth*MeV,     charge, 
			 J,              +1,             0,          
			 0,               0,             0,             
...
// function to calculate the width of a state based on the lifetime
// lifetime argument should be in ns
// returns an energy in MeV
// G4double CalcWidth(G4double lifetime) {
//   G4double width;
//   if (lifetime < DBL_MAX && lifetime >= 0.00) {
//     G4double hbar_MeV_ns = 6.58217e-16 * 1e-6 * 1e9; //hbar_ev_s * (1 MeV/1e6 eV) * (1e9 ns/1 s)
//     width = hbar_MeV_ns / lifetime;
//   } else {
//     width = 0.0;
//   }
//   return width;
